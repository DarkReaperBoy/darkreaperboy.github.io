...
function createOrb(a, b) {
	...
	orbs.push({
		x:a, y:b, lastX:a, lastY:b, hue:0, colorAngle:0, angle:f+Math.PI/2, size:rand(1, 3)/2, centerX:cw/2, centerY:ch/2, radius:e, speed:rand(1, 5)/1e3*(e/750)+.005, alpha:1-Math.abs(e)/cw, draw:function() {
...
var loop=function() {
	...
	for(var a=orbs.length; a--; )for(var b=orbs[a], c=3; c--; )b.update(),
	b.draw(ctx)
}
